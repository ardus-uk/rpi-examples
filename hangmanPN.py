...
def players_char(prompt_string):
    ...
    while (message !=""):
        ...
        if (len(character)!=1): message = "One letter only, please.\n" + prompt_string
        elif ((ord(character) not in range(65,91)) and (ord(character) not in range(97,123))): message = "Only alphabetic characters, please.\n" + prompt_string
        else: message = ""
    return character.upper()
    
def word_from_wordlist(filename):
    with open(filename,'r') as reader: whole_text = reader.read()
